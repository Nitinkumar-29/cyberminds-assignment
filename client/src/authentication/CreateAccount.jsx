...
const CreateAccount = () => {
  ...
  const handleCreateAccount = (e) => {
    e.preventDefault();
    if (createAccountCredentials.length !== 0) {
      createAccount();
    }
  };
  return (
      ...
      <div className="flex flex-col items-center justify-center space-y-6 w-full">
        ...
        >
          <div className="flex flex-col space-y-3 w-1/2">
            ...
            <div className="flex items-center justify-between px-2 border-[1px] border-black rounded-md">
              <input
                ...
                required
              />
              <span className="cursor-pointer" onClick={handleTogglePasswordType}>
                {passwordType === "password" ? <FaEye /> : <FaEyeSlash />}
              </span>
            </div>
          </div>
          <button type="submit" className="px-4 py-2 rounded-md border-black bg-black hover:bg-white text-white hover:text-black duration-200 border-[1px] w-1/2">
            Submit
          </button>
          ...
          <div className="flex items-center justify-center space-x-4">
            ...
            <Link to="/login" className="text-blue-600 hover:underline hover:underline-offset-2 font-semibold">
              Login
            </Link>
          </div>
        </form>
